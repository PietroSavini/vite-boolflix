            ...
            <div class="row">
                <div v-if="store.movies && store.series !== [] && store.movies[store.active]" class="col-4">
                    <ul v-if="store.type === 'movie'" class="details mt-3">
                        <li>
                            <h2 class="jumbo-title">{{ store.movies[store.active].title }}</h2>
                        </li>
                        <li>
                            <h3 class="jumbo-original-title mb-4">{{ store.movies[store.active].original_title }}</h3>
                        ...
                        <li>
                            <p class="jumbo-description">{{ store.movies[store.active].overview }}</p>
                        </li>
                    </ul>
                    <ul v-if="store.type === 'tv'" class="details mt-3">
                        <li>
                            <h2 class="jumbo-title">{{ store.series[store.active].name }}</h2>
                        </li>
                        <li>
                            <h3 class="jumbo-original-title mb-4">{{ store.series[store.active].original_name }}</h3>
                        ...
                        <li>
                            <p class="jumbo-description">{{ store.series[store.active].overview }}</p>
                        </li>
                    </ul>
                ...
                <div v-if="store.movies && store.series !== [] && store.movies[store.active]" class="col-8">
                    <img v-if="store.type === 'movie'"
                        :src="`${store.jumboImgUrl}${store.movies[store.active].backdrop_path}`" alt="">
                    <img v-if="store.type === 'tv'" :src="`${store.jumboImgUrl}${store.series[store.active].backdrop_path}`"
                        alt="">
    ...
    <section class="results">
        ...
        <div class="container">
            ...
            <div class="row media-scroller mb-3 flex-nowrap align-items-center">
                <!-- cards for movies -->
                <AppCardMovie @click="store.active = index, store.type = 'movie'" v-for="(movieCard, index) in store.movies"
                    :movie="movieCard" />
            </div>
            <h2>TV-Series</h2>
            <div class="row media-scroller flex-nowrap align-items-center mb-3">
                <AppCardTv @click="store.active = index, store.type = 'tv'" v-for="(tvCard, index) in store.series"
                    :tv="tvCard" />
